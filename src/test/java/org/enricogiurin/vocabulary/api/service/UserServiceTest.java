...
class UserServiceTest {
  ...
  void findOrCreate() {
    ...
    verify(emailService, never()).notifyAdmin(any(), any());
  }

  @Test
  void findOrCreate_notExisting() {
    //given
    assertThat(userRepository.findUserIdByKeycloakId(NEW_USER_KEYCLOAK_ID)).isEmpty();
    //when
    ...
    verify(keycloakClientService, never()).createNewUser(any(), any());
    verify(emailService).notifyAdmin(any(), any());
  }
